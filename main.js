...
// CODE WAR KYU 7 (BUT IT IS FUNNY)
// Description:
...
sc = (screws) => screws.split('').reduce((time, screw, i) => (screw == screws[i + 1] ? time + 2 : time + 7), 1) - 7

sc = (screws) => {
  let sum = (2 * screws.length) - 1
   for (let i = 0; i < screws.length - 1; i++) {
       if (screws[i] != screws[i + 1]) { sum += 5 }
     }
    return sum
  }
// CODE WAR KYU 6
...
function calcPrice(percent, num) {
  return ((100 - percent) * num / 100)
}
...
function nbMonths(startPriceOld, startPriceNew, savingperMonth, percentLossByMonth) {
  ...
  if (oldcar >= newcar) return [0, oldcar - newcar]

  while (avail <= 0) {
    oldcar = calcPrice(loss, oldcar)
    newcar = calcPrice(loss, newcar)

    avail = (oldcar + save) - newcar
  ...
  return [countMonth - 1, Math.round(avail)]
}
// ANOTHER SOLUTIONS
...
nbMonths = (myCar, newCar, perMonth, loss) => {
  loss = 1 - (loss / 100)
  var w = myCar
  for (var m = 0; w < newCar; m++) {
    loss -= m % 2 == 0 ? 0 : 0.005
    myCar *= loss
    newCar *= loss
...
function balance(book) {
  book = book.replace(/[^a-zA-Z0-9. \n]/gi, '').split(/[\n,]/)
  let prev = +book[0],
      prs = [],
      price = +book[0]
      final = `Original Balance: ${ price.toFixed(2) }\r\n`
  for (let i = 1; i < book.length; i++) {
    let str = ''
    if (book[i] === '') continue
    let arr = book[i].split(' ')
    price = +arr[2]
    prev = (prev - +arr[2]).toFixed(2)
    prs.push(+arr[2])
    str = `${arr[0] } ${ arr[1] } ${ price.toFixed(2) } Balance ${ prev }\r\n`
    final += str
  }
  let total = prs.reduce((a, b) => a + b)
  final += `Total expense  ${ total.toFixed(2) }\r\nAverage expense  ${ (total / prs.length).toFixed(2)}`
  return final
}
...
balance = (book) => {
  ...
  entries = entries.map((row) => row.replace(/\S+$/g, (m) => {
